...
class SettingsViewModel: ObservableObject {
    ...
    func addVideoWithScope(
           ...
           // 1. Heute eintragen — über UseCase
           do {
       ...
       func removeScheduleWithScope(
           ...
           let videoId = schedule.video?.id
           let groupId = schedule.recurrenceGroupID

           // 1. Schedule löschen — über UseCase
           do {
           ...
           // 2. Falls allFuture: zukünftige Schedules löschen
           guard scope == .allFuture, videoId != nil else { return }
           ...
           print("🗑️ Lösche \(futureSchedules.count) zukünftige Schedules")
           futureSchedules.forEach { modelContext.delete($0) }
           try? modelContext.save()
       }

       // applyTemplatesToActiveDays — progressVM Parameter entfernen
       func applyTemplatesToActiveDays(for user: User) {
    ...
    var weeklyTemplates: [VideoSchedule] {
        let userId = session.currentUser?.id
        let descriptor = FetchDescriptor<VideoSchedule>(
            predicate: #Predicate<VideoSchedule> { s in
                s.isTemplate == true &&
                s.isWeeklyTemplate == true  // ← nur Wochenplan
            }
        )
        let all = (try? modelContext.fetch(descriptor)) ?? []
        return all.filter { $0.templateUserId == userId }
    ...
    func addDailyTemplate(
        ...
        template.isWeeklyTemplate = isWeekly
        template.planMode = isWeekly ? "weekly" : "daily"
    ...
    func applyWeekPlan(
        ...
        print("📅 startDate rein: \(startDate)")
        print("📅 start berechnet: \(Calendar.current.startOfDay(for: startDate))")
        let calendar = Calendar.current
                ...
                if isActiveDay && hasMinutes {
                    for (index, planned) in dailyVideos.enumerated() {
                        let schedule = VideoSchedule(
                            ...
                            sets: planned.sets,                          // 🆕
                            reps: planned.reps,                          // 🆕
                            expertPauseSeconds: planned.expertPauseSeconds,  // 🆕
                            recurrenceRule: .daily,
                            recurrenceGroupID: groupId
                        )
                        ...
                        schedule.weightKg = planned.weightKg             // 🆕
    ...
    func recurrenceRule(for date: Date) -> RecurrenceRule {
        return preferences.activePlanMode
    }
    ...
    //Wechselmodus
    func setActiveMode(_ rule: RecurrenceRule) {
        preferences.activeMode = rule.planMode
        do {
            try modelContext.save()
