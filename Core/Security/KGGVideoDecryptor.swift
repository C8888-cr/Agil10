...
struct KGGVideoDecryptor {
    ...
    func decrypt(sourceURL: URL, using key: SymmetricKey) throws -> Data {
    ...
    func readMetadata(from fileURL: URL) throws -> (version: UInt8, chunkSize: Int) {
    ...
    func openInputStream(for fileURL: URL) throws -> InputStream {
        guard let input = InputStream(url: fileURL) else {
            throw DecryptorError.sourceNotReadable
        }
        return input
    ...
    func decryptChunk(_ sealed: Data, using key: SymmetricKey) throws -> Data {
