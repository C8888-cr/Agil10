...
/*
  ...
  */
    ...
    // ✅ NEUE Bottom Bar mit blauem Balken (nur im Training-Modus)
    private var bottomControls: some View {
        VStack(spacing: 12) {
            // ✅ Timeline
            ZStack(alignment: .bottomTrailing) {
            ...
            // ✅ BLAUER BALKEN (mittig, wie TrainingOverlayView)
            if let progress = viewModel.trainingProgress,
               viewModel.settings.mode == .training,
               !progress.isInPause {
                HStack(spacing: 16) {
...
#Preview("VideoPlayerView - Normal Mode") {
...
#Preview("Training Center Overlay - Aktiv") {
    ZStack {
        Color.black.ignoresSafeArea()
...
#Preview("Training Center Overlay - Pause") {
    ZStack {
        Color.black.ignoresSafeArea()
        ...
        VStack {
            ...
            VStack(spacing: 12) {
                ...
                HStack(spacing: 20) {
                    ...
                    Menu {
                        Text("1.0x").tag(PlaybackSpeed.normal)
                        Text("1.25x").tag(PlaybackSpeed.fast)
                        Text("1.5x").tag(PlaybackSpeed.veryFast)
                    } label: {
                        HStack(spacing: 4) {
                            Text("1.0x")
                            Image(systemName: "chevron.up.chevron.down")
...
#Preview("Progress Bar Interactive") {
    ZStack {
        ...
        VStack(spacing: 20) {
            Text("Progress Bar - Drag zum Spulen")
                .font(.headline)
                .foregroundStyle(.white)
            ...
            GeometryReader { geometry in
                ...
                ZStack(alignment: .leading) {
                    ...
                    Capsule()
                        .fill(Color.white.opacity(0.5))
                        .frame(width: safeWidth * 0.7, height: 4)
                    
                    Capsule()
                        .fill(Color.white)
                        .frame(width: safeWidth * 0.5, height: 4)
                }
                .frame(width: safeWidth, height: 4)
            }
            .frame(height: 20)
